...
class PolicyIterationAgent:
    def __init__(self, gamma=0.80, theta=1e-6):
    ...
    def train(self, environment, full_game_episodes=10):
        ...
        for episode in range(full_game_episodes):
            ...
            print(f"Episode {episode + 1}/{full_game_episodes} completed. Total Reward: {total_reward}, Lines Cleared: {environment.game.matris.lines}", flush=True)
        ...
        for rotation in range(4):  # Explore all 4 possible rotations
            for horizontal_shift in range(-MATRIX_WIDTH, MATRIX_WIDTH): 
                ...
                try:
                    ...
                    # Drop the tetromino
                    reward = simulated_game.hard_drop()
                    ...
                    print(f"Simulated Rotation: {rotation}, Shift: {horizontal_shift}, Reward: {reward}")  # Debug statement
...
def evaluate_policy(agent, game, episodes=10):
    ...
    for episode in range(episodes):
        ...
        print(f"Episode {episode + 1}/{episodes}: Total Reward: {reward}, Lines Cleared: {game.matris.lines}", flush=True)
...
if __name__ == "__main__":
    ...
    # Train the agent
    agent.train(environment, full_game_episodes=4)

    # Evaluate the trained policy
    evaluate_policy(agent, game, episodes=4)
